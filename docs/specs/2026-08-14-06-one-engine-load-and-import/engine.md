...
Boot is the first run — every file is dirty against an empty previous snapshot. Later store changes (edits, imports via ingest) re-trigger the same run, debounced at 1 s with a 30 s max wait, replacing the three debounces it retires (embeddings 5 s, corpus 30 s, regions 5 s / 30 s max). Short, because a pass over clean files is predicate checks only — no network.

One pass runs at a time. A notification arriving mid-pass requests a rerun rather than starting a second pass or being dropped — the coalescing `runChain` pattern from the regions sync, not the embeddings sync's `syncing` guard, which silently loses the notification. The previous-snapshot reference advances only when its pass completes, so a file edited mid-pass diffs dirty again on the rerun instead of falling through the gap.
...
A file dirty for one stage but clean for another emits settled-skip for the clean stages. Skip is an explicit event, never silence (spec.md decision).

### Dependencies

Injected at start, mirroring the deps style of the three current reconcilers (`EmbeddingSyncDeps`, `CorpusSyncDeps`, `RegionSyncDeps` in `app/lib/regions/sync-types.ts`) — the engine's deps are their union:
...
- `ready` — resolves when the first pass settles (pool drained plus tail step). Boot gating awaits it exactly as `app/routes/project.tsx` awaits the three handles today; the loading screen dismisses on it per spec.md's boot-gating pin.
...
The engine owns this contract; spec.md and both view components link here. Delivery is the `onEvent` dep, supplied before the first pass begins — a listener wired at start cannot miss boot events. Fan-out to multiple views is the init layer's concern, not the engine's; the engine calls one function and never imports React.
...
- `skipped` — on `settled` only: `true` when the stage's predicate found nothing to do, absent when work happened
- `error` — on `failed` only: a message for the console and the import row's warning state

Lifecycle per stage: `queued` (all three stages, emitted when the file enters the pool) → `working` (the stage starts) → `settled` or `failed`. Every queued stage terminates in exactly one `settled` or `failed` — "settled with nothing to do" is `settled` with `skipped: true`, so the UI works for stages that leave no persistent mark (spec.md). No timeout machinery: a stage that never reports is a bug and belongs in the console.

A file is done when all three stages have terminated. Views derive everything from the stream: the boot view counts settled/failed per stage against queued; the import UI advances a row per stage and shows spec.md's "imported, processing incomplete" warning on any `failed`. Events are an internal contract between the engine and its own init wiring — not hostile input, no validation layer on them.

Granularity is the stage, nothing finer. A large file embedding many chunk batches is `working` from its first batch to its last; the per-chunk progress the old `onProgress` callbacks carried does not survive, because neither view shows it — the boot view counts files per stage, the import row shows a stage label. The tail step (description sync, language statistics) emits no events at all: it is corpus-scoped, has no file to attribute work to, and no view shows it — it is simply inside `ready` and inside every pass. Re-settling is idempotent by contract: an echo pass re-emits queued → working → settled-skip for a file a view already showed as done, and views must treat that as no news, not a regression.
...
The file stays in the store (spec.md: a fixer failure is "imported, processing incomplete", never un-import). Nothing is written for the failed stage, so its predicate stays dirty and the next pass retries it — no retry timer, the next pass is simply the next store notification (often the same file's own successful stages patching it). The regions stage keeps its three-strikes write quarantine (`MAX_CONSECUTIVE_WRITE_FAILURES` in `app/lib/regions/sync.ts`): a write that can never apply must not spin the engine; a quarantined file emits `failed` and stops being retried until its content changes.
...
- **Embeddings** (`app/lib/embeddings/sync.ts`): reused — `prepareFile` (chunk, parse companion, `diffChunks`), `batchBySize`, `fetchEmbeddingBatch` with its short-answer guard, `settleUnchangedCompanions`' per-file companion reconciliation, deleted-file companion cleanup. Retired — `tagNeededChunks`/cross-file batch accumulation (per-file embedding now, spec.md decision), the module's own subscribe/debounce/run loop and `onProgress`.
...
**Skeleton.** The engine's slice of spec.md's walking skeleton: boot the demo project on the engine — loading screen driven by engine events, dismissing when `ready` resolves — then drop one small markdown file and watch its queue row advance Embedding → Classifying → Finding regions → Added off the same stream. This is where the echo loop either converges or runs away; it is built and verified before anything below.

**Contract.** Given/when/then against a running engine, riskiest first:

- _Convergence (the pinned no-op-boot case from spec.md)._ Given a project whose files are fully processed — companions current, content hashes matching, regions scanned — when the engine's first pass runs, then no embedding or model call is made, every stage emits settled-skip, and no second round of work follows the engine's own writes. Given a file that _does_ need work, when its pass completes and the echo pass runs, then the echo pass is settled-skips only and no third pass fires.
...
- _Retry._ Given that failed file, when the next pass triggers, then the failed stage runs again (its predicate is still dirty) while the stages that succeeded settle as skips.
- _Explicit skip._ Given a file with no prose to embed, when finalize runs, then embed emits settled-skip (and keeps no companion), and classify and regions each terminate per their own predicates — an empty excerpt classifies nothing, but the event still arrives.
...
**Isolation.** The engine run with a faked store (an in-memory `FileStore` behind the injected access functions) and faked stage functions — embed, classify, region find/mark — each honoring its real contract (embed writes a companion from the chunks it was given; classify patches attributes with the content hash; regions records scanned units). Assertions land only on the event stream and the store writes: event ordering and terminality per stage, skip flags, error propagation, companion/attributes/regions content after a pass, and that a second pass over the faked store produces skips and no writes. No assertion reaches into engine internals.
