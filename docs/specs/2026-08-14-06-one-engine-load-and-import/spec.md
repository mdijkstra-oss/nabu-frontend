...
- Status is event-driven only. The engine reports each file's lifecycle per stage, and "settled with nothing to do" is an explicit event, never silence — emitted once per (file, stage) pair when it first settles; a pair that stays settled and clean is silent on later passes — so the UI works for stages that leave no persistent mark. No timeout machinery: a stage that never reports is a bug and belongs in the console.
- A file that fails before the store (unreadable, not markdown, corrupt structure) was never imported: a red terminal not-imported state. A file that fails in a fixer stage is already in the project and stays there: a warning imported-but-processing-incomplete state; the engine retries on its next pass. Wording for both states lives in [import-ui.md](import-ui.md)'s status table.
...
2. Drag one `.md` file containing a heading and two sentences onto the project. The queue row appears, runs migrate/normalize through ingest, then advances through the three stage statuses to "Added" as the engine reports — row labels per [import-ui.md](import-ui.md)'s status table.
...
- A migration that rewrites a synced file persists the rewritten form back to the server, and initial load otherwise never echoes files back (`withoutPersist`) — unpinned today; a test pinning migration persist-back and `withoutPersist` suppression is written against today's sync path before the refactor starts and kept green after, same treatment as the no-op boot case below.
- Unsupported dropped files show "Not supported" — the label is pinned by the import stories — and are never written to the store; the never-written half is unpinned today and gets its pinning test against today's import path before the refactor starts, kept green after.
...
- **Given** a project whose files are already fully processed (companions current, hashes matching, regions scanned), **when** the app boots, **then** the engine's first pass performs no per-file stage work — no chunk embedding, no classification calls, no region detection — and the loading screen's stage rows settle without work. The corpus tail (description sync, `app/lib/corpus/sync-descriptions.ts`) behaves exactly as today and may embed topic labels, so the test asserts per-file silence, not total network silence. This is the reconcilers' current no-op boot; the test is written against today's three reconcilers before the refactor starts and kept green against the engine after. Pre-refactor it can assert embedding silence (through the `EmbeddingSyncDeps.fetchBatch` spy, which the corpus tail does not use) and region silence (`detect` is injectable today); classification has no injection point until the engine's `classify` seam exists, so that third of the assertion joins the test post-refactor — see the convergence case in [engine.md](engine.md).
