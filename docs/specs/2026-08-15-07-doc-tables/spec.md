...
- One real DuckDB table per block, named after the block id with hyphens as underscores: block `table-3k2j9x1a` owns `table_3k2j9x1a` (`table_<id>` as shorthand throughout this spec). No file part in the name — moves and renames must not break queries. System-generated ids cannot collide; hand-authored ids that collide outright or only after derivation degrade to last-writer-wins per [projection.md](projection.md).
...
- Callout block rendering: pinned by the stories under `app/lib/editor/callout-blocks/`. Hidden-block behavior has no story or test pinning it, so it gets a case here before any component builds: given a document containing a hidden-renderer block (`json-attributes`), when the editor renders it, then no visible node appears — unchanged by the renderer union gaining `"table"`.
