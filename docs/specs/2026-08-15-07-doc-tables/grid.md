...
| `caption`         | `string \| undefined`                  | caption line below the grid, chart-card style                                        |
...
| `onAddColumn`     | `(index) => void`                      | column insertion affordance                                                          |
...
The card's only state is the draft value of the currently focused cell and which header popover is open. The block prop is the source of truth for everything else.
...
- **Insertion points**: hovering the boundary between two columns (in the header) or two rows (at the row edge) reveals a `+` affordance at that spot; the ends of the grid always offer one. Adding a column inserts it at that index with a generated default name (`Column n`, deduped per table-block.md) and immediately opens its header popover so the user names it; the key is generated from the final committed name per table-block.md's rule. Adding a row inserts an empty row (all columns `""`).
...
3. **Key generation on add-column with duplicate names** — given an existing column named "Score" (key `score`), when a column is added and committed with the name "Score", then the new column's key is the deduped variant per table-block.md's rule, the existing column's key is unchanged, and rows are untouched (no key for the new column until cells are filled, or the empty-string fill — whichever table-block.md specifies).
...
8. **Add row / add column at chosen spot** — the insertion affordance between row 1 and row 2 produces a block whose new empty row sits at index 1; likewise for columns.
