...
The design assumption is that recall is cheap and precision is expensive. Retrieval casts wide with two independent retrievers and ten query rewrites, then spends model calls narrowing — first at chunk granularity, then at sentence granularity.

## Chunks

Chunking is the unit of identity for the whole system. Search results, embedding cache entries and analysis candidates all address content by chunk hash, which only works because exactly one function produces them:

```ts
export const chunkFileForEmbedding = (content: string): Chunk[] => chunkText(extractProse(content))
```

The source is always the file's prose with its JSON blocks stripped, never the raw file and never a rendered view. Chunks are 250 tokens with 20% overlap, and their boundaries are adjusted twice: back to the nearest whitespace so words stay whole, then out to the nearest sentence boundary when one lies within 300 characters. Each chunk is identified by a content hash of its final text.
...
A natural-language intent is turned into ten hypothetical passages — text that would plausibly appear in a document that answers the query — and each is embedded and searched independently. The ten are drawn across five angles:
...
Each hypothetical passage produces a ranked list by cosine similarity, and the full-text retriever produces its own. All of them are combined by reciprocal rank fusion:
...
Fusion is on rank, not score, so a cosine similarity and a BM25 score never have to be made commensurable. A chunk that places tenth for six of the ten passages outranks one that places first for a single passage — which is the behaviour wanted, since a chunk matching many angles of the question is more likely to be about it.

The fused candidate pool is the larger of a thousand chunks or a fifth of the corpus. It is deliberately generous; narrowing happens later, with a model in the loop.
...
Neither is a real DuckDB function. Before execution, a resolver extracts the tokens, generates and embeds the hypothetical passages, runs fusion, and rewrites the query against the resulting chunk set. `EMBEDDINGS_FROM_FILE('code.md')` does the same using a codebook entry's own text as the query, which is what the analysis pass uses to find candidate spans for a code.

The consequence is that the agent composes retrieval and structured filtering in one expression — semantic search restricted to documents with a given tag and date range is a `WHERE` clause, not a pipeline the agent has to orchestrate. It also means the agent never handles a vector.
...
**Verdict** is the only stage that calls a model, and the only one that streams. Batches of ten hits are judged concurrently, and each batch's results run the remaining stages immediately rather than waiting for the rest — results appear progressively.

Within a batch, two filters run in sequence. The scout sees whole chunks and answers only which are irrelevant; it is coarse, cheap and framework-aware. What survives goes to the semantic filter, which returns sentence ranges rather than a verdict:
...
Passages are presented with per-hit letter prefixes and numbered sentences, so a reference names both which hit and which sentences. Filtering therefore also does extraction: the pipeline learns not just that a hit is relevant but which sentences carry the relevance, and `trim` uses exactly those ranges. Each hit's verdict is cached by intent and content, so repeating or refining a search re-judges only what actually changed.
...
**Extend** grows the returned byte range to cover any annotation that overlaps it and appends those annotations to the hit's text. A search result therefore arrives carrying the coding already applied to it, which is what lets the agent reason about what is coded and what is not without a second query.
...
Alongside search, documents are classified into a type and a subject, and both label sets are consolidated: labels are embedded, compared pairwise by cosine similarity, and merged with union-find so that near-duplicate phrasings collapse to one representative. Classification is skipped when a document's content hash is unchanged.

The result is a browsable topic tree and, more importantly, a set of corpus descriptions passed into query expansion — so hypothetical passages are generated with knowledge of what the corpus actually contains.
